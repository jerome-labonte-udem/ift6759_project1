...
train_images = train_images[:train_size]  # train_size examples should be chosen at random here
val_images = val_images[:val_size]  # val_size examples should be chosen at random here

# Normalize pixel values to be between 0 and 1
train_images, val_images = train_images[:train_size] / 255.0, val_images[:val_size] / 255.0
...
train_metadata = get_metadata(df, "BND", "2010-05-01", "2010-07-01")[:train_size]
...
"""
metadata contain in order : CLEARSKY_GHI, DAYTIME (bool), day of year, hour, minute
the method get_metada should be modified to change the type of metadas if needed
"""
train_metadata = get_metadata(df, "BND", "2010-05-01", "2010-07-01")[:train_size]
val_metadata = get_metadata(df, "BND", "2010-07-01", "2010-09-01")[:val_size]

train_labels = get_labels(df, "BND", "2010-05-01", "2010-07-01")[:train_size]
val_labels = bnd_labels = get_labels(df, "BND", "2010-07-01", "2010-09-01")[:val_size]
...
model.build([(None, train_images.shape[1], train_images.shape[2], train_images.shape[3]),
...
plt.plot([train_clearsky_mse] * len(history.history['output_1_mse']), label="clearsky_mse")
plt.xlabel('Epoch')
